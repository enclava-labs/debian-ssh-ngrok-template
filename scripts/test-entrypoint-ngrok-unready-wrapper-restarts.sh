...
IMAGE_TAG="${IMAGE_TAG:-debian-ssh-ngrok-template:ngrok-unready-wrapper-test}"

docker build -t "$IMAGE_TAG" .

stub_dir="$(mktemp -d)"
container_name="debian-ssh-ngrok-unready-wrapper-$$"
cleanup() {
...
if ! docker exec "$container_name" grep -qxF 'ssh -p 5555 user@127.0.0.1' /home/user/health/ssh.txt 2>/dev/null; then
    ...
    echo "expected initial ngrok endpoint before wrapper restart test" >&2
    exit 1
fi
...
echo "expected wrapper to restart the entrypoint and recover after ngrok unready exit" >&2
exit 1
