...
IMAGE_TAG="${IMAGE_TAG:-debian-ssh-ngrok-template:wrapper-unready-watchdog-test}"

docker build -t "$IMAGE_TAG" .

stub_dir="$(mktemp -d)"
container_name="debian-ssh-ngrok-wrapper-watchdog-$$"
cleanup() {
...
docker run -d --name "$container_name" \
    ...
    -e DEBIAN_NGROK_UNREADY_ACTION=exit \
    -e DEBIAN_SSH_RESTART_WRAPPER=1 \
    -e DEBIAN_SSH_RESTART_WRAPPER_CHECK_SECONDS=1 \
...
if ! docker exec "$container_name" grep -qxF 'ssh -p 5555 user@127.0.0.1' /home/user/health/ssh.txt 2>/dev/null; then
    ...
    echo "expected initial ngrok endpoint before wrapper watchdog test" >&2
    exit 1
fi
...
echo "expected wrapper watchdog to restart the entrypoint after post-ready unready state" >&2
exit 1
