...
IMAGE_TAG="${IMAGE_TAG:-debian-ssh-ngrok-template:ngrok-cache-restart-test}"

docker build -t "$IMAGE_TAG" .

stub_dir="$(mktemp -d)"
container_name="debian-ssh-ngrok-cache-restart-$$"
cleanup() {
...
root="$(mktemp -d)"
mkdir -p "$root/api" "$HOME/.cache/ngrok"

if [ -f "$HOME/.cache/ngrok/stopped-endpoint" ]; then
    cat >"$root/api/tunnels" <<'JSON'
{"tunnels":[]}
JSON
else
    if [ ! -f /tmp/ngrok-initial-started ]; then
...
JSON
        (
            sleep 4
            touch "$HOME/.cache/ngrok/stopped-endpoint"
            cat >"$root/api/tunnels" <<'JSON'
{"tunnels":[]}
JSON
        ) &
...
if ! docker exec "$container_name" grep -qxF 'ssh -p 5555 user@127.0.0.1' /home/user/health/ssh.txt 2>/dev/null; then
    ...
    echo "expected initial ngrok endpoint before cache restart test" >&2
    exit 1
fi
...
echo "expected ngrok restart to clear stale cache state and recover" >&2
exit 1
